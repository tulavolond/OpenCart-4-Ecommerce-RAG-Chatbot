...
def load_datasets(product_path, order_path):
    """
    Загрузка датасетов с продуктами и заказами.
    """
    try:
        # Загружаем данные
        product_df = pd.read_csv(product_path)
        ...
        print(f"Продуктов загружено: {len(product_df)} строк")
        print(f"Заказов загружено: {len(order_df)} строк")
        
        # Заполняем NaN значения в зависимости от типа столбца
        for df in [product_df, order_df]:
            # Текстовые столбцы (object) заполняем пустой строкой
            object_cols = df.select_dtypes(include=['object']).columns
            df[object_cols] = df[object_cols].fillna('')
            
            # Числовые столбцы заполняем нулём
        ...
        print(f"Ошибка: {e}")
        print("Убедитесь, что файлы с данными находятся в папке 'data/'")
        return None, None
